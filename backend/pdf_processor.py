...
def simple_vector_representation(text):
    ...
    This is a placeholder for when you can't use OpenAI's embeddings.
    It creates a basic frequency-based vector.
    """
    # Handle empty text
...
def process_pdfs():
    ...
    try:
        ...
        for batch_idx in range(0, len(pdf_files), 2):
            batch_files = pdf_files[batch_idx:batch_idx + 2]
            batch_documents = []
            
            # Process each PDF in this small batch
            for pdf_file in tqdm(batch_files, desc=f"Batch {batch_idx//2 + 1}/{(len(pdf_files)+1)//2}"):
                try:
                    # Extract text
                    ...
                    for i, chunk in enumerate(chunks):
                        ...
                        vector = simple_vector_representation(chunk)
                    ...
                    print(f"Memory error processing {pdf_file}, skipping...")
                    continue
                except Exception as e:
        ...
        print(f"Created {total_chunks} document chunks from {len(pdf_files)} PDF files.")
...
def initialize_vector_db():
    ...
    if os.path.exists(vector_file):
        try:
            with open(vector_file, 'r') as f:
                documents = json.load(f)
            print(f"Loaded {len(documents)} document chunks from vector database.")
            return documents
        except Exception as e:
...
if __name__ == "__main__":
    # For testing
    documents = initialize_vector_db()
    if documents:
